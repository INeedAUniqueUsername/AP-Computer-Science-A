	...
	public void blur(int x, int y, int w, int h) {
		...
		for(int r = y; r < y+h; r++) {
			for(int c = x; c < x+w; c++) {
				...
				int nearbyPixels = 1, nearbyRed = p.getRed(), nearbyGreen = p.getGreen(), nearbyBlue = p.getBlue();
				for(Pixel p_nearby : new Pixel[]{
						getPixel(c+1, r),
						getPixel(c-1, r),
						getPixel(c, r-1),
						getPixel(c, r+1),
				...
				result[r-y][c-x] = new Color(nearbyRed, nearbyGreen, nearbyBlue);
			}
		}
		for(int r = 0; r < h; r++) {
			for(int c = 0; c < w; c++) {
